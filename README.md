...
L1. [简单]3.1.1/2 + 3.2.1 + 3.3.1/2 + 3.4.1  <-- 在做：
	...
	2. 需要加入输入和输出。现在脚本的语法还没有确定，主要是还没有具体应用，只有通过具体应用才能发现哪种方式是最好的。
	3. 在main函数中设定输出和输出。作为脚本可以输入，但是没有实现交互模式，以及输出。我想做到Model中无论哪种关系，怎么建立起来的，都可以输出成脚本。
		首先所有的高级模块，比如model/process之类的，现在用代码固定，后面将用注册的方式，这样可以通过任意组合来实现不同的功能，现在还没有必要。
