...
【根据项目的需求，抽象、完善和精炼需求，形成符合逻辑的、完整的、完备的需求说明】
...
所以函数的核心是建立系统内各个部分的“关系”，只是这种关系大部分是函数运行出来的，而不是明显的设定。
这个工具就是想翻转这种情况，先“描述关系”，然后再自动的、可定制的使用动作来实现关系。

【描述是什么，而不是如何、怎么做】
既然是“描述关系”，那么手段是描述，而不是具体的说明以何种方式实现，而是静态的说明关系如何。
...
用户需要的功能：
1. 可以设定对象的关系，并显示关系图，并进行修改等操作。<br/>
	比如将关系图显示为 graphviz 图像。
2. 根据关系图，可以查询。<br/>
	比如 查询一个函数的调用关系，并且找到其中包含“rpmb”的字的函数。
3. 根据关系图，可以建立实际的系统，并运转起来。<br/>
...
【设定了基本的需求后，要创造一种新的理论或者用已有的理论来设计系统。】
...
#### 概括说明
* object 是一个内容的抽象代表，比如一个数字，一个字符串，503室的居民，我的体重。它必须是具有意义的。
* relation 是内容之间的关系，是静态的，是描述的。
* intructation 是对于内容的改变，是动作的，是明确的、可操作的。

比如 `c=a+b`。那么 `a/b/c` 都是object，而 “=+” 则是描述三者之间的关系。
无论 `a/b/c` 怎么变化，都必须满足的关系。
intructation 则是“a赋值1”，“b赋值2”，那么按照关系“c赋值是3”。
是实现关系的动作，或者建立object、relation的动作。

#### 详细说明

1. 对象(Object)，是系统中概念的最基本类型，关系约定的是对象(Object)。对象是个“标记“事物，是被关系影响的目标。它不需要复杂的功能。
	1. 需要一个名字，用来标明自己，在不同的区域中可能会避免重名，或者有特殊的字符要求。
	2. 允许指向多个关系，这些关系都是和此对象有联系的。
2. 系统的核心是关系（Relation），用于描述多个对象之间的联系，而且关系本身也是一种对象。
	1. 关系不是函数（function），函数是动作，可以建立、删除、修改“对象”。
...
5. 系统在定好的关系中，也可以按照关系进行运转（perform，不用运行"run"这个词），这就能够将此系统应用在实际的项目中。
	...
	**注意：OR-Net是运转的模板，运转需要建立此模板的具体实例，以及将需要的变量都设置具体的值才可以运转。**
	1. 运转有两个实现方法，
		* 一个是 map 等relation 内部有特定实现，当涉及到的对象发生变化后，就会运转起来，需要建立的是“影响延伸算法”。
		* 另外，从外部建立算法，用 `visitor` 模式来实现，实现隔离算法和关系图(OR-Net)。【应该用第二种！】
		目前需要考虑复杂的OR网时，关系的运转问题！比如复杂的公式计算，目前希望用观察者模式解决这个问题。
		`改变 -> 联动 -> 求值 -> 改变 -> 联动 -> 求值 ...`
		希望建立简洁、完备的逻辑。
	2. 运转有两种情况，
		* 一种是整个系统运转起来后，当一部分对象发生变化后，引发了其他的对象发生变化。
			每种变化都是由自变量(variables)决定的一个系统状态(state)。
...
object-relation 的结构化体系说明：
1. `obj1 -- rlt1 --> obj2` : 最基本的OR。
	【现实中，所有的模型都给出对象之间的关联，但是关联可以是固定的，或者在某些条件下改变。
	如果将条件也算在关系中，那么关联就是固定的。】
	1. rlt 也是 obj。
	2. rlt 是有方向的，而且 from 和 to 涉及到的对象可以有多个。
	3. obj 可以有多个 relation。
	4. obj 最终要有一组对象是可以获取“内容”的，内容可以是数字、文字等具体的东西。
		否则就会造成无限延展的概念。但是并不是所有的object都应该有内容，
		所以只有特定的Object才能够有内容。
		1. 我的叔叔年龄是40。`O(我) -- R(叔叔) --> O(某人) -- R(年龄) >  Integer(40)`
2. 多个OR组成了“OR-Net”。
	```
	o ──┐
	o ──┴──> r ─┬───> o
	...
	上面就是OR-Net的普通泛型。
	1. OR-Net 是一个复杂的结构体。
	2. 模板：根据类型来构造一个对象。约束：检查一个对象是否符合类型的要求。
		1. Relation类型或模板：是一个描述，可以统括一个OR-Net，然后对外只公开对外Object。
		2. Object类型和模板：是一个描述，通过一个OR-Net， 标明都有哪些属性【？】
3. `from OR-Net --rlt--> to OR-Net`，rlt可以将 OR-Net 和另外一组 OR-Net 建立联系。
	1. rlt 如果是“单向的”，那么就可以根据“从OR-Net”得到“目的OR-Net”。
	2. rlt 如果是 filter，那么可以根据
	3. rlt 如果是 Map，就是可以将 from 的property 映射为 to 的 property。
		`to.a = from.abc & to.b = from.xyz`
4. OR-Net运转起来（OR-Net::perform）。所谓的“运转”是按照OR-Net设定的关系运行。
	运转起来可能需要两个步骤，一个是将抽象模型映射为具体的对象模型，然后在具体对象模型中运行。
	1. 现实中，一个系统会先构造各种模型，模型的关系用业内已经成熟的经验来指定，然后根据模型来构筑实际可以运行的程序，最后再运行起来程序。	所以最开始的OR-Net也是抽象的，然后通过映射（Map）到一个具体系统的实现上，比如“永久存储”，那么映射到具体的实现上就可能是“文本文件”或者“DB”。
		```
		存储 <--具体-- 文本文件 or 存储 <--具体-- database
		日志 --保存--> 存储 <--具体-- 文本文件、database
		```
		以上的情况在程序设计中非常常见，设计人员会先给出一个描述，然后再根据描述中的要求到目标系统中找有哪些技术符合要求，最后指定一个最符合要求的答案。
	2. 如果使用者不管抽象的描述，而是直接就用具体的实现，那么就需要考虑运转的流程了。运转的流程是：
		1. 每个对象（非relation）都是一个独立的运行体。每个对象都必须是可以自运行的，因为OR-Net并不知道如何运行。
		2. 然后当对象改变时，会引发关系维持，从而带动目标对象的改变。相当于一个“观察者模式”。
		比如显示OR-Net的graphviz，就是直接指定了具体的显示方法，而不是先给出抽象的需求，然后再说明具体的实现。
	3. 例子： `c^2 = a^2 + b^2`
		1. 原始的 OR-Net 可能只是一个模板，需要转换成可以运转的OR-Net。这个叫做可运转化(performable)。
			1. 可以运转的OR-Net中每个O都必须可以运转的。
			2. 不缺少需要的对象值，这里需要a和b的值。
		2. 运转(perform)，是将模板变成具体的实例，比如根据模板建立实例(instance)，然后a和b赋值后，开始求值。
			上面的公式
		3. 模板关系 --运转化--> 程序 --运转--> 实例，然后实例就可以在“运转容器”中运转了。
...
* 指令是完成某件事情的步骤，目的是按照步骤就可以实现某个目标。
	所以指令必须是明确的、无歧义的。
* 描述是说多个对象之间的关系，可以是明确的步骤，也可能是某种约束条件。
	目的是描述联系，只要有联系就行。

举个例子：
* `c=sqrt(a^2 + b^2)` 就是“求C值的一个明确步骤”，当然也可是三个对象之间的关系。
* `c^2= a^2 + b^2` 就是三个对象之间的关系，通过公式的逆推，可以得到一个指令（一系列指令）从其他对象计算得到某元素的值。
* `c > a + b` 这个就是一个关系，是一个“限制”，即使a和b的值都确定了，也无法计算得到c的值，所以它不是一个指令，而是一个限制条件。
...
为了建立关系模型，需要有“指令”，指令就是描述“干什么”，这样就可以搭建一个关系模型。
和其他程序的区别就是：指令建立的是关系，不是只设定最终的结果，
比如:
```python
s = a+b+c
```
...
2. 表达式:
多个对象和关系可以形成一个组合对象(combined object)，此组合对象对外的关系就是内部所有对象对外关系。
当然，多个关系也可以组合成一个关系(combined relation=combined element)。
可以将关系和对象组成一个有意义的组合，形成一个“组合对象“，这个就是表达式。
...
组合是将多个对象合成一个对象，有两个范例：
* OR-Net 合成一个Object，那么隐藏了OR-Net中的关系等细节，认为是一个对象，可以连接其他的relation。
	`<o> --r--> b --r--> <o>`，b是合成的。
* OR-Net 合成一个Relation，也是隐藏了OR-Net中的对象和关系细节，认为是一个关系，可以对外连接多个对象。
	`<o> --e--> <o>`，e是合成的。
...
运转准确的说，不是一个层，而是一个重要的功能，目前定为两种方案
* 有一个对象叫做 “perform”，包含在容器(container)中的OR-Net在其中运行。
* 有一个visitor工具，超出 OR-Net的模型，在用户下达“运转”的命令后，visitor就会访问OR-Net，
	然后运转起来，运转本身不是OR-Net的一部分。
...
举例子来说，
1. “动作”添加各种对象和关系，建立了简单的关系网"x=(y+z)/3"。
2. 然后关系网运转时，给y和z赋值后，x就得到结果。
3. “动作”添加了一个对象，内部有复杂的关系图，然后建立一个Graphviz的对象，
4. 建立“show”关系关联两个对象。这个时候，包含多个对象和“show”关系的图就建立。
5. 系统运行的时候，show会将第一个对象的内部的关系告诉Graphviz对象。
6. Graphivz对象就会显示出关系的图。
7. 如果对象的内部关系发生变化后，那么show会通知Graphviz更新，
  这样就保持了两个对象之间的show关系。
...
1. misc 是多种工具，可以供其他的层使用。是最底层的、不能对象化的模块。
	比如基本日志输出等。
2. native 层，用于保存各种功能的底层实现，供 `Object` 和 `Relation` 使用。
	之后有可能会化为对象或relation。
3. core 是核心，`Object+Ralation`数据结构在这里，以及相关的模块。
	比如 `Object、Relation` 等类。
4. ext 是在core的基础上扩展的各个 `Object` 和 `Relation` 的子类，主要支持各种语法和规则，形成语言层的 `OR-Net`。
	比如数学计算、文字处理、表达式等。还有 `运转` 相关的。
	还有core之外的容器，处理流程之类的OR。
5. fwk 集成 `Object+Relation` 的扩展模块，主要是用于构成应用程序，所以大部分是和操作系统、画面等相关的模块。
	比如显示、日志、流程处理等的 `object`、`relation`。
6. app 非 `Object+Relation` 的模块，用于建立的主程序和测试程序。
...
## 任务清单
1. 实现简单的应用，比如函数调用关系图，以及数据结构的关系图，或者模块的分布图。
