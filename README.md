...
intructation 是对于内容的改变。
比如 c=a+b。那么 a/b/c 都是object，而 “=+” 则是描述三者之间的关系。
无论a/b/c 怎么变化，都必须满足的关系。
...
4. OR-Net::perform，OR-Net运转起来。所谓的“运转”是按照OR-Net设定的关系运行。
	运转的思路是：每个对象（非relation）都是一个独立的运行体，然后当对象改变时，会引发关系维持，
	从而带动目标对象的改变。相当于一个“观察者模式”。
	每个对象都必须是可以自运行的，因为OR-Net并不知道如何运行。
	【现实中，一个系统会先构造各种模型，模型的关系用业内已经成熟的经验来指定，
	然后根据模型来构筑实际可以运行的程序。然后再运行起来程序。】
	【显示OR-Net的graphviz并不是运转，它是关系的一种普通维护`？`】
	`c^2 = a^2 + b^2`
