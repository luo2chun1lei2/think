...
6. 问题
	...
	2. 思考一下，是否需要“标签”这个东西？
	3. 双引号问题：所有的文字可以加入双引号，这样内部的空格也可以包含，但是放入模型时，需要将左右的双引号去掉。
	4. 定义一个元素时，名字需要唯一，但是描述也许需要很长，所以应该拆成两个部分，一个是用于标记的name，一个是用于显示的“desc”(description)。
	5. 执行的语法没有说明，在大量的加入属性后，无法让用户简单的了解怎么使用。
	6. 命令行语句，还不支持换行“\”。
	7. 允许截取大模型中的一部分关系，这个是查询的功能，然后形成新的模型，然后映射到"显示"中。
