...
L1. [简单]3.1.1/2 + 3.2.1 + 3.3.1/2 + 3.4.1  <-- 在做：构筑了简单的Model+Element+Relation，还没有输入和输出，就是没有办法使用。命令的格式需要再仔细想一想。
...
5. 问题
	...
	2. 现在偏离了使用，目前最要紧的是制作函数调用关系，以及模块包含关系图，其他的功能如果没有用到，就先不要实现。
