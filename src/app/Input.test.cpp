...
TEST_CASE("test input about comand line", "[app]")
{
	...
	input.add(Element elm1("elm1"); Element elm2("elm2"); Element elm3("elm3"); Relation rlt0("rlt0");	// not in model.
		  Relation rlt1("rlt1"); Relation rlt2("rlt2");
		  Model model("model"); SECTION("Add elements.") {
		  model.add_elm(&elm1);
		  model.add_elm(&elm2);
		  model.add_elm(&elm3);
		  ...
		  found = model.find_elm("elm1"); REQUIRE(found.size() == 1);
		  REQUIRE(found[0] == &elm1); found = model.find_elm("rlt2");
		  REQUIRE(found.size() == 1); REQUIRE(found[0] == &rlt2);
		  found = model.find_elm("xxx"); REQUIRE(found.size() == 0);}
