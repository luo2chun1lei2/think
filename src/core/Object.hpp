 ...
 * 包含功能：
 * * 名字
 *   1. 有一个名字，作为ID识别，不保证唯一性。
 *   2. 名字的作用范围是有区域的，限于连接的对象。
 * * 关系
 *   1. 包含多个关系，这些关系都和此对象有联系。但是并不是说有此关系一定会修改这个对象的状态。
 * * 值 * 【IValue是说目前所有的对象都有的特性】
 *   1. 每个对象都可以读取和设置值，不过并不是所有的对象都可以设置值，或者获取值。
 * * subject-observer : 是 subject-observer 的接受方(observer)。 【未实现】
 */
